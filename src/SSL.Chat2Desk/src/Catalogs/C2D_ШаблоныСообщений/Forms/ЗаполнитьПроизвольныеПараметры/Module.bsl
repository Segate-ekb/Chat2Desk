...
Процедура ДобавитьЭлементыФормыПараметровШаблона(Шаблон)
	...
	Иначе
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	C2D_ШаблоныСообщенийПараметры.Ссылка,
		|	C2D_ШаблоныСообщенийПараметры.ИмяПараметра КАК Имя,
		|	C2D_ШаблоныСообщенийПараметры.ТипПараметра КАК Тип,
		|	C2D_ШаблоныСообщенийПараметры.ПредставлениеПараметра КАК Представление
		|ИЗ
		|	Справочник.C2D_ШаблоныСообщений.Параметры КАК C2D_ШаблоныСообщенийПараметры
		|ГДЕ
		|	C2D_ШаблоныСообщенийПараметры.Ссылка = &Ссылка";
