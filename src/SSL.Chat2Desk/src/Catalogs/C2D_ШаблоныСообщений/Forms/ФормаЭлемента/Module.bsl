...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПолноеИмяТипаПараметраВводаНаОсновании.СписокВыбора.Добавить(C2D_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий(),
	...
	Если Параметры.Ключ = Неопределено Или Параметры.Ключ.Пустая() Тогда
		...
		Если Параметры.ЗначениеКопирования = Справочники.C2D_ШаблоныСообщений.ПустаяСсылка() Тогда
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ПустаяСтрока(Объект.ПолноеИмяТипаПараметраВводаНаОсновании) Тогда
		Объект.Назначение = C2D_ШаблоныСообщенийКлиентСервер.ОбщийПредставление();
		Объект.ПредназначенДляВводаНаОсновании = Ложь;
		Объект.ПолноеИмяТипаПараметраВводаНаОсновании = C2D_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Объект.ПредназначенДляВводаНаОсновании Тогда
		...
		Объект.Назначение = C2D_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий();
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Иначе
		...
		Если ТаблицаСоответствийИменВложенийИдентификаторам.Количество() > 0 Тогда
			...
			ДокументHTML = C2D_ШаблоныСообщенийСлужебный.ПолучитьОбъектДокументHTMLИзТекстаHTML(ТекущийОбъект.ТекстШаблонаПисьмаHTML);
			ЗаменитьИменаКартинокНаИдентификаторыПочтовыхВложенийВHTML(ДокументHTML, ТаблицаСоответствийИменВложенийИдентификаторам);
			ТекущийОбъект.ТекстШаблонаПисьмаHTML = C2D_ШаблоныСообщенийСлужебный.ПолучитьТекстHTMLИзОбъектаДокументHTML(ДокументHTML);
...
Функция ОбработатьТекстШаблона()
	...
	Если Объект.ПредназначенДляSMS Тогда
		Результат.ОбычныйТекст = 
			C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(Результат.ОбычныйТекст, ДеревоПараметровШаблона, ВариантПреобразования);
		ПроверяемыйТекст = Результат.ОбычныйТекст;
	Иначе
		Если Результат.ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
			
			ВложенияВHTML = Новый Структура();
			ТелоПисьмаВHTML.ПолучитьHTML(Результат.HTMLТекст, ВложенияВHTML);
			
			Результат.HTMLТекст = C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(СтрЗаменить(Результат.HTMLТекст, "&quot;", """"), ДеревоПараметровШаблона, ВариантПреобразования);
			Результат.ОбычныйТекст = C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТелоПисьмаВHTML.ПолучитьТекст(), ДеревоПараметровШаблона, ВариантПреобразования);
		...
		Иначе
			...
			КонецЕсли;
			Результат.ОбычныйТекст = C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(Результат.ОбычныйТекст,
		...
		Результат.ТемаПисьма = C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(Результат.ТемаПисьма, ДеревоПараметровШаблона, ВариантПреобразования);
	...
	ПараметрыШаблонаИзТекста = C2D_ШаблоныСообщенийСлужебный.ПараметрыТекстаСообщения(ПроверяемыйТекст);
	
	Для каждого ПараметрШаблонаИзТекста Из ПараметрыШаблонаИзТекста Цикл
		
		ОписаниеПараметра =  C2D_ШаблоныСообщенийСлужебный.ИмяПараметраБезСтрокиФормата(ПараметрШаблонаИзТекста.Ключ);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Пока Индекс >= 0 Цикл
		СтрокаТаблицыВложений = C2D_ШаблоныСообщений.СтрокаВложений(Вложения.Получить(Индекс));
		...
		ИначеЕсли СтрокаТаблицыВложений.Статус = "ВнешнийНовый" Тогда
			...
			СсылкаНаФайл = C2D_ШаблоныСообщенийСлужебный.ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(ТекущийОбъект.Ссылка, СтрокаТаблицыВложений, ИмяФайла, 0);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	Оповестить("Запись_C2D_ШаблоныСообщений", Объект.Ссылка, ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл" И ТипЗнч(Источник) = Тип("СправочникСсылка.C2D_ШаблоныСообщенийПрисоединенныеФайлы") Тогда
...
Процедура ПолноеИмяТипаПараметраВводаНаОснованииПриИзменении(Элемент)
	Если ПустаяСтрока(Объект.ПолноеИмяТипаПараметраВводаНаОсновании) Тогда
		Объект.ПолноеИмяТипаПараметраВводаНаОсновании = C2D_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий();
	КонецЕсли;
	Объект.ПредназначенДляВводаНаОсновании = (Объект.ПолноеИмяТипаПараметраВводаНаОсновании <> C2D_ШаблоныСообщенийКлиентСервер.ИдентификаторОбщий());
...
Процедура РеквизитыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ИспользоватьПроизвольныеПараметры Тогда
		...
		ОткрытьФорму("Справочник.C2D_ШаблоныСообщений.Форма.ПроизвольныйПараметр", ПараметрыФормы,,,,, ОповещениеОЗакрытие);
	КонецЕсли;
КонецПроцедуры
...
Процедура РеквизитыПередУдалением(Элемент, Отказ)
	Если ИспользоватьПроизвольныеПараметры Тогда
		...
		Если СтрНачинаетсяС(ТекущиеДанные.Имя, C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()) Тогда
			Отбор = Новый Структура("ИмяПараметра", Сред(ТекущиеДанные.Имя, СтрДлина(C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()) + 2));
		Иначе
...
Процедура ИзменитьВложение(Команда)
	...
	Если ТекущиеДанные.Ссылка = ПредопределенноеЗначение("Справочник.C2D_ШаблоныСообщенийПрисоединенныеФайлы.ПустаяСсылка") Тогда
...
Процедура ПослеЗакрытияФормыПараметра(ОписаниеПараметра, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(ОписаниеПараметра) = Тип("Структура") Тогда
		...
		Иначе
			...
			Если СтрНачинаетсяС(Реквизит.Имя, C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()) Тогда
				Отбор = Новый Структура("ИмяПараметра", Сред(Реквизит.Имя, СтрДлина(C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()) + 2));
			Иначе
...
Процедура ИнициализироватьНовыйШаблонСообщений(Знач НастройкиШаблоновСообщений)
	...
	Если ЗначениеЗаполнено(Параметры.ПолноеИмяТипаОснования)
		 И C2D_ШаблоныСообщенийСлужебный.ОбъектЯвляетсяПредметомШаблона(Параметры.ПолноеИмяТипаОснования) Тогда
	...
	ИначеЕсли Параметры.ПараметрыВыбора.Количество() > 0 Тогда
		...
		ИначеЕсли Параметры.ПараметрыВыбора.Свойство("ПредназначенДляChat2Desk")
			И Параметры.ПараметрыВыбора.ПредназначенДляChat2Desk Тогда
			ВидСообщения = "Chat2Desk"	
		КонецЕсли;
	...
	Если Параметры.Основание = Неопределено Тогда
		...
		Если ВидСообщения = "СообщениеSMS" Тогда
			...
			Объект.ПредназначенДляChat2Desk = Ложь;
		ИначеЕсли ВидСообщения = "Chat2Desk" Тогда 
			Объект.ПредназначенДляSMS = Ложь;
			Объект.ПредназначенДляЭлектронныхПисем = Ложь;
			Объект.ПредназначенДляChat2Desk = Истина;
			Объект.ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.ОбычныйТекст;
		Иначе 
			Объект.ПредназначенДляSMS = Ложь;
			Объект.ПредназначенДляЭлектронныхПисем = Истина;
			Объект.ПредназначенДляChat2Desk = Ложь;
			Объект.ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML;
		КонецЕсли;
	...
	Иначе
		...
		Если Не ШаблонСформирован Тогда
			...
			Если Объект.ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
...
Процедура ШаблонНаОснованииДокументаВзаимодействия()
	...
	Если Объект.ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
		...
		Сообщение = C2D_ШаблоныСообщенийСлужебный.КонструкторСообщения();
		Сообщение.Текст = Объект.ТекстШаблонаПисьмаHTML;
		C2D_ШаблоныСообщенийСлужебный.ОбработатьHTMLДляФорматированногоДокумента(ПараметрШаблона, Сообщение, Истина, ВложенияПисьма);
		СтруктураВложений = Новый Структура();
...
Процедура УстановитьТекстШаблона(ТекущийОбъект, СписокФайлов = Неопределено)
	...
	Если ТекущийОбъект.ПредназначенДляSMS Тогда
		ТелоСообщенияОбычныйТекст.УстановитьТекст(
			C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТекущийОбъект.ТекстШаблонаSMS, СписокВсехПараметров, "ПараметрыВПредставление"));
		
	Иначе
		Если ТекущийОбъект.ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
			УстановитьHTMLДляФорматированногоДокумента(
				C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТекущийОбъект.ТекстШаблонаПисьмаHTML, СписокВсехПараметров, "ПараметрыВПредставление"), ТекущийОбъект.Ссылка, СписокФайлов);
		Иначе
			ТелоСообщенияОбычныйТекст.УстановитьТекст(
				C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТекущийОбъект.ТекстШаблонаПисьма, СписокВсехПараметров, "ПараметрыВПредставление"));
		КонецЕсли;
		
		Объект.ТемаПисьма = C2D_ШаблоныСообщенийСлужебный.ПреобразоватьТекстШаблона(ТекущийОбъект.ТемаПисьма, СписокВсехПараметров, "ПараметрыВПредставление");
...
Процедура ОтображениеЭлементовФормы(ФорматаПисьма = "")
	...
	Иначе
		СуффиксЗаголовка = НСтр("ru = 'Шаблон сообщения Chat2Desk'");
	...
	Элементы.ГруппаДоступ.Видимость = 
		НЕ ПараметрыДоступа("Изменение", Метаданные.Справочники.C2D_ШаблоныСообщений, "Ссылка").ОграничениеУсловием;
...
Процедура УстановитьHTMLДляФорматированногоДокумента(ТекстШаблонаПисьмаHTML, СсылкаНаТекущийОбъект, СписокФайлов = Неопределено)
	...
	C2D_ШаблоныСообщенийСлужебный.ОбработатьHTMLДляФорматированногоДокумента(ПараметрШаблона, Сообщение, Истина, СписокФайлов);
	СтруктураВложений = Новый Структура();
...
Процедура СформироватьСписокРеквизитовИПечатныхФорм()
	...
	ПараметрыШаблона = C2D_ШаблоныСообщенийСлужебный.ПараметрыШаблона(Объект);
	СведенияОШаблоне = C2D_ШаблоныСообщенийСлужебный.СведенияОШаблоне(ПараметрыШаблона);
...
Процедура ЗаполнитьДеревоРеквизитов(Приемник, Источник, ЭтоОбщиеИлиПроизвольныеРеквизиты = Неопределено)
	...
	Для Каждого СтрокаДерева Из Источник.Строки Цикл
		...
		Если ЭтоОбщиеИлиПроизвольныеРеквизиты = Неопределено Тогда
			Если СтрокаДерева.Имя = C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()
				ИЛИ СтрокаДерева.Имя = C2D_ШаблоныСообщенийСлужебный.ЗаголовокОбщиеРеквизиты() Тогда
...
Процедура УстановитьЭлектронноеПисьмоHTML(ТребуетсяПереносТекста = Ложь)
	...
	Объект.ТипТекстаПисьма = ПредопределенноеЗначение("Перечисление.C2d_СпособыРедактированияЭлектронныхПисем.HTML");
...
Процедура УстановитьЭлектронноеПисьмоОбычныйТекст(ТребуетсяПереносТекста = Ложь)
	...
	Объект.ТипТекстаПисьма = ПредопределенноеЗначение("Перечисление.C2d_СпособыРедактированияЭлектронныхПисем.ОбычныйТекст");
	Если ТребуетсяПереносТекста Тогда
		...
		Для каждого Вложение Из СписокФайлов Цикл
			Если ПустаяСтрока(Вложение.ИДФайлаЭлектронногоПисьма) Тогда
				...
				Исключение
					...
					ЗаписатьОшибкуВЖурналРегистрации(ИмяСобытияC2D_ШаблоныСообщений(), ИнформацияОбОшибке, НСтр("ru = 'Не удалось извлечь и записать присоединенный файл для копирования'"));
...
Процедура УдалитьПрисоединенныйФайл(ПрисоединенныйФайл)
	...
	ЭлементБлокировки = Блокировка.Добавить("Справочник.C2D_ШаблоныСообщенийПрисоединенныеФайлы");
...
//  ТипТекстаПисьма  - Перечисление.C2d_СпособыРедактированияЭлектронныхПисем - для определения необходимости выполнения преобразований.
...
Процедура СохранитьКартинкиФорматированногоДокументаКакПрисоединенныеФайлы(Ссылка, ТипТекстаПисьма,
	...
	Если ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ФормаВидТекстаЭлектронногоПисьма);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПредназначенДляChat2Desk");
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(ИмяСобытия, ИнформацияОбОшибке, ТекстСобытия)
	...
	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.C2D_ШаблоныСообщений,, Комментарий);
	
КонецПроцедуры

&НаСервере
Функция ИмяСобытияC2D_ШаблоныСообщений()
	
	Возврат НСтр("ru = 'C2D Шаблоны сообщений'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьШаблонПоВнешнейОбработке()
		...
		Иначе
			...
			Если СтруктураДанныхВнешнейОбработки.ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
				УстановитьЭлектронноеПисьмоHTML(Истина);
...
Процедура ОчиститьШаблон(Форма)
	...
	Форма.Объект.ТипТекстаПисьма                        = ПредопределенноеЗначение("Перечисление.C2d_СпособыРедактированияЭлектронныхПисем.ОбычныйТекст");
...
Процедура ДобавитьВТекстСообщенияПараметр()
	...
	Если Элементы.Реквизиты.ВыделенныеСтроки <> Неопределено Тогда
		...
		Для каждого НомерСтроки Из Элементы.Реквизиты.ВыделенныеСтроки Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если Объект.ТипТекстаПисьма = ПредопределенноеЗначение("Перечисление.C2d_СпособыРедактированияЭлектронныхПисем.HTML") Тогда
