...
Процедура ЭтаФормаИнтерфейсИнит(ВсеПоддерживаемыеТипы)
	...
	Иначе
		ЭтаФормаЗаголовок = НСтр("ru = 'Выбор справочников, для сопоставления клиентов'; en = 'Edit data type'"); 
...
Процедура ЭтаФормаПараметрыПрочитатьНаСервере()
	...
	Выборка = РегистрыСведений.C2D_ТипыКлиентов.Выбрать();
...
Процедура ПерезаписатьИспользуемыеТипы(ОписаниеТипов)
	РегистрыСведений.C2D_ТипыКлиентов.ПерезаписатьИспользуемыеТипы(ОписаниеТипов);
КонецПроцедуры
