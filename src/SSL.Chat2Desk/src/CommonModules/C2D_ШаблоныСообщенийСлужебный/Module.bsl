...
Функция СформироватьСообщение(ПараметрыОтправки) Экспорт
	...
	Если ПараметрыОтправки.Шаблон = Справочники.C2D_ШаблоныСообщений.ПустаяСсылка() Тогда
	...
	Иначе
		...
		Иначе
			ПараметрыОтправки.ДополнительныеПараметры.Вставить("ВидСообщения", "Chat2desk");
		КонецЕсли;
	КонецЕсли;
	...
	// Вложения
	Если (ПараметрыШаблона.ТипШаблона = "Письмо" или ПараметрыШаблона.ТипШаблона = "Chat2Desk") И СведенияОШаблоне <> Неопределено Тогда
	...
	C2D_ШаблоныСообщенийПереопределяемый.ПриФормированииСообщения(Сообщение, ПараметрыШаблона.ПолноеИмяТипаНазначения, ПараметрыОтправки.Предмет, ПараметрыШаблона);
	Попытка
	...
	Если не ПараметрыШаблона.ТипШаблона = "SMS" И ПараметрыШаблона.ФорматПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
...
Функция СформироватьСообщениеИОтправить(ПараметрыОтправки) Экспорт
	...
	ИначеЕсли ПараметрыОтправки.Шаблон.ПредназначенДляЭлектронныхПисем Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
			...
			Если Сообщение.ДополнительныеПараметры.ФорматПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
		...
		КонецЕсли
	Иначе
		МассивСсылок = справочники.C2D_Вложения.СохранитьВложенияДляОтправки(Сообщение.Вложения);
		Для каждого Получатель Из Сообщение.Получатель Цикл
			СтруктураПараметров = новый структура("client_id", Получатель.Значение);
			Если МассивСсылок.Количество() = 0 Тогда
				СтруктураПараметров.Вставить("text", Сообщение.Текст);
				СоответствиеОтвет = C2D_ВзаимодействиеСApi.POST_messages(СтруктураПараметров);
				...
				ИначеЕсли СоответствиеОтвет.Получить("status") = "error" тогда
					...
					Результат.ОписаниеОшибки = C2D_ВзаимодействиеСApiСлужебный.СформироватьТекстОшибки(СоответствиеОтвет);
					Прервать;
				КонецЕсли;
				
			Иначе
				ЭтоПервоеВложение = Истина;
				Для каждого Вложение Из МассивСсылок Цикл
					Если ЭтоПервоеВложение Тогда
						СтруктураПараметров.Вставить("text", Сообщение.Текст);
					КонецЕсли;
					СтруктураПараметров.Вставить("attachment", Справочники.C2D_Вложения.ПолучитьСсылкуВложения(Вложение.Наименование));
					СоответствиеОтвет = C2D_ВзаимодействиеСApi.POST_messages(СтруктураПараметров);
					Если СоответствиеОтвет.Получить("status") = "success" Тогда
						Результат.Отправлено = истина;
					ИначеЕсли СоответствиеОтвет.Получить("status") = "error" тогда
...
Функция ИсточникиШаблоновСообщений() Экспорт
	...
	МассивТиповИлиМетаданных = РегистрыСведений.C2D_ТипыДанныхШаблонов.ПолучитьТипыДанных().Типы();
...
Функция ИспользуютсяC2D_ШаблоныСообщений() Экспорт
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьC2D_ШаблоныСообщений");
КонецФункции
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "C2D_ШаблоныСообщенийСлужебный.ДобавитьРольДобавлениеИзменениеЛичныхШаблоновВПрофилиСБазовымиПравами";
	Обработчик.РежимВыполнения = "Оперативно";
	
	ЧитаемыеОбъекты = Новый Массив;
	ЧитаемыеОбъекты.Добавить("Справочник.C2D_ШаблоныСообщений");
	...
	Обработчик.Процедура       = "Справочники.C2D_ШаблоныСообщений.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.C2D_ШаблоныСообщений.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ОчередьОтложеннойОбработки = 2;
	Обработчик.ЧитаемыеОбъекты    = СтрСоединить(ЧитаемыеОбъекты, ",");
	Обработчик.ИзменяемыеОбъекты  = "Справочник.C2D_ШаблоныСообщений";
	Обработчик.БлокируемыеОбъекты = "Справочник.C2D_ШаблоныСообщений";
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.Справочники.C2D_ШаблоныСообщений, Истина);
	Списки.Вставить(Метаданные.Справочники.C2D_ШаблоныСообщенийПрисоединенныеФайлы, Истина);
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	Описание = Описание + "
	|Справочник.C2D_ШаблоныСообщений.Чтение.Пользователи
	|Справочник.C2D_ШаблоныСообщений.Изменение.Пользователи
	|";
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		Описание = Описание + "
			|Справочник.C2D_ШаблоныСообщенийПрисоединенныеФайлы.Чтение.Пользователи
			|Справочник.C2D_ШаблоныСообщенийПрисоединенныеФайлы.Изменение.Пользователи
			|";
	КонецЕсли;
...
Функция ШаблонПоВладельцу(ВладелецШаблона) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	C2D_ШаблоныСообщений.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.C2D_ШаблоныСообщений КАК C2D_ШаблоныСообщений
		|ГДЕ
		|	C2D_ШаблоныСообщений.ВладелецШаблона = &ВладелецШаблона";
	...
	Иначе
		Возврат Справочники.C2D_ШаблоныСообщений.ПустаяСсылка();
	КонецЕсли;
...
Функция СведенияОШаблоне(ПараметрыШаблона) Экспорт
	...
	ИмяУзлаОбщихРеквизитов = C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов();
...
Функция КонструкторСведенийОШаблоне()
	...
	НастройкиШаблоновСообщений = C2D_ШаблоныСообщенийСлужебныйПовтИсп.ПриОпределенииНастроек();
...
Процедура ОпределитьСписокРеквизитовИВложений(СведенияОШаблоне, ПараметрыШаблона)
	...
	Если ЗначениеЗаполнено(ПараметрыШаблона.ПолноеИмяТипаНазначения) Тогда
		...
		C2D_ШаблоныСообщенийПереопределяемый.ПриПодготовкеШаблонаСообщения(РеквизитыОбъектаНазначения, СведенияОШаблоне.Вложения, ПараметрыШаблона.ПолноеИмяТипаНазначения, ПараметрыШаблона);
...
Функция ОбъектЯвляетсяПредметомШаблона(ПолноеИмяТипаНазначения) Экспорт
	...
	Результат = C2D_ШаблоныСообщенийСлужебныйПовтИсп.ПриОпределенииНастроек().ПредметыШаблонов.Найти(ПолноеИмяТипаНазначения, "Имя");
...
Функция СообщениеБезШаблона(ПараметрыОтправки)
	...
	ПараметрыШаблона = C2D_ШаблоныСообщенийКлиентСервер.ОписаниеПараметровШаблона();
...
Функция ОпределитьПредметыШаблонов() Экспорт
	...
	ТипыПредметовШаблоновСообщений = РегистрыСведений.C2D_ТипыДанныхШаблонов.ПолучитьТипыДанных().Типы();
...
Функция ВидыШаблонов() Экспорт
	...
	ТипыШаблонов.Добавить("Chat2Desk", НСтр("ru = 'Сообщения через C2D'"));
...
Функция ПодготовитьЗапросДляПолученияСпискаШаблонов(ТипШаблона, Предмет = Неопределено, ВладелецШаблона = Неопределено, ВыводитьОбщиеШаблон = Истина) Экспорт
	...
	Если ТипШаблона = "SMS" Тогда
		...
		ПредназначенДляChat2Desk = Ложь;
	ИначеЕсли ТипШаблона = "Письмо" Тогда 
	...
	Иначе
		ПредназначенДляSMS = Ложь;
		ПредназначенДляЭлектронныхПисем = Ложь;
		ПредназначенДляChat2Desk = Истина;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	C2D_ШаблоныСообщений.Ссылка,
	|	C2D_ШаблоныСообщений.Представление,
	|	C2D_ШаблоныСообщений.Наименование КАК Имя,
	|	C2D_ШаблоныСообщений.ВнешняяОбработка КАК ВнешняяОбработка,
	|	C2D_ШаблоныСообщений.ШаблонПоВнешнейОбработке КАК ШаблонПоВнешнейОбработке,
	|	ВЫБОР
	|		КОГДА C2D_ШаблоныСообщений.ПредназначенДляЭлектронныхПисем
	|			ТОГДА ВЫБОР
	|					КОГДА C2D_ШаблоныСообщений.ТипТекстаПисьма = ЗНАЧЕНИЕ(Перечисление.C2d_СпособыРедактированияЭлектронныхПисем.HTML)
	|						ТОГДА C2D_ШаблоныСообщений.ТекстШаблонаПисьмаHTML
	|					ИНАЧЕ C2D_ШаблоныСообщений.ТекстШаблонаПисьма
	|				КОНЕЦ
	|		ИНАЧЕ C2D_ШаблоныСообщений.ТекстШаблонаSMS
	|	КОНЕЦ КАК ТекстШаблона,
	|	C2D_ШаблоныСообщений.ТипТекстаПисьма,
	|	C2D_ШаблоныСообщений.ТемаПисьма,
	|		ВЫБОР
	|			КОГДА КОЛИЧЕСТВО(C2D_ШаблоныСообщений.Параметры.ИмяПараметра) > 0
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	...
	|	Справочник.C2D_ШаблоныСообщений КАК C2D_ШаблоныСообщений
	|ГДЕ
	|	(C2D_ШаблоныСообщений.ТолькоДляАвтора = ЛОЖЬ ИЛИ C2D_ШаблоныСообщений.Автор = &Пользователь) И
	|	C2D_ШаблоныСообщений.ПредназначенДляSMS = &ПредназначенДляSMS
	|	И C2D_ШаблоныСообщений.ПредназначенДляЭлектронныхПисем = &ПредназначенДляЭлектронныхПисем
	|	И C2D_ШаблоныСообщений.ПредназначенДляChat2Desk = &ПредназначенДляChat2Desk
	|	И C2D_ШаблоныСообщений.Назначение <> ""Служебный""
	|	И &ОтборПоВладельцуШаблона
	|	И C2D_ШаблоныСообщений.ПометкаУдаления = ЛОЖЬ";
	
	Если ЗначениеЗаполнено(ВладелецШаблона) Тогда
		ОтборПоВладельцу = "C2D_ШаблоныСообщений.ВладелецШаблона = &ВладелецШаблона";
		Запрос.УстановитьПараметр("ВладелецШаблона", ВладелецШаблона);
	Иначе
	...
	Запрос.УстановитьПараметр("ПредназначенДляChat2Desk", ПредназначенДляChat2Desk);
	Запрос.УстановитьПараметр("Пользователь", Пользователи.АвторизованныйПользователь());
	
	ОтборОбщиеШаблоны = ?(ВыводитьОбщиеШаблон, "C2D_ШаблоныСообщений.ПредназначенДляВводаНаОсновании = ЛОЖЬ", "");
	
	Если ЗначениеЗаполнено(Предмет) Тогда
		Запрос.Текст = Запрос.Текст + " И (C2D_ШаблоныСообщений.ПолноеИмяТипаПараметраВводаНаОсновании = &ПолноеИмяТипаПредмета "
...
//
// Параметры:
//   Владелец - СправочникСсылка.C2D_ШаблоныСообщений
//   Сведения - см. C2D_ШаблоныСообщенийКлиентСервер.СтрокаВложений
//   ИмяФайла - Строка
//   Размер - Число 
...
//
// Возвращаемое значение:
//   см. C2D_ШаблоныСообщенийКлиентСервер.ОписаниеПараметровШаблона
//
Функция ПараметрыШаблона(Шаблон) Экспорт
	
	Результат = C2D_ШаблоныСообщенийКлиентСервер.ОписаниеПараметровШаблона();
	
	Если ТипЗнч(Шаблон) = Тип("ДанныеФормыСтруктура") Или ТипЗнч(Шаблон) = Тип("СправочникОбъект.C2D_ШаблоныСообщений") Тогда
		
		УстановитьПараметрыШаблона(Шаблон, Результат);
		
	ИначеЕсли ТипЗнч(Шаблон) = Тип("СправочникСсылка.C2D_ШаблоныСообщений") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	C2D_ШаблоныСообщений.ПредназначенДляВводаНаОсновании,
		|	C2D_ШаблоныСообщений.ПолноеИмяТипаПараметраВводаНаОсновании,
		|	C2D_ШаблоныСообщений.Назначение,
		|	C2D_ШаблоныСообщений.Наименование,
		|	C2D_ШаблоныСообщений.ПредназначенДляЭлектронныхПисем,
		|	C2D_ШаблоныСообщений.ТипТекстаПисьма,
		|	ВЫБОР
		|		КОГДА C2D_ШаблоныСообщений.ПредназначенДляЭлектронныхПисем
		|			ТОГДА ВЫБОР
		...
		|					ИНАЧЕ C2D_ШаблоныСообщений.ТекстШаблонаПисьма
		|				КОНЕЦ
		|		КОГДА C2D_ШаблоныСообщений.ПредназначенДляChat2Desk
		|			ТОГДА ВЫБОР
		|					КОГДА C2D_ШаблоныСообщений.ТипТекстаПисьма = ЗНАЧЕНИЕ(Перечисление.C2d_СпособыРедактированияЭлектронныхПисем.HTML)
		|						ТОГДА C2D_ШаблоныСообщений.ТекстШаблонаПисьмаHTML
		|					ИНАЧЕ C2D_ШаблоныСообщений.ТекстШаблонаПисьма
		...
		|	КОНЕЦ КАК ТекстШаблона,
		|	C2D_ШаблоныСообщений.ТемаПисьма,
		|	C2D_ШаблоныСообщений.УпаковатьВАрхив,
		|	C2D_ШаблоныСообщений.ТранслитерироватьИменаФайлов,
		|	C2D_ШаблоныСообщений.ФорматВложений,
		|	C2D_ШаблоныСообщений.ПредназначенДляSMS,
		|	C2D_ШаблоныСообщений.ОтправлятьВТранслите,
		|	C2D_ШаблоныСообщений.ПодписьИПечать,
		|	C2D_ШаблоныСообщений.ШаблонПоВнешнейОбработке,
		|	C2D_ШаблоныСообщений.ВнешняяОбработка,
		|	C2D_ШаблоныСообщений.Ссылка,
		|	C2D_ШаблоныСообщений.ВладелецШаблона
		|ИЗ
		|	Справочник.C2D_ШаблоныСообщений КАК C2D_ШаблоныСообщений
		|ГДЕ
		|	C2D_ШаблоныСообщений.Ссылка = &Шаблон
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	C2D_ШаблоныСообщенийПечатныеФормыИВложения.Идентификатор,
		|	C2D_ШаблоныСообщенийПечатныеФормыИВложения.Имя
		|ИЗ
		|	Справочник.C2D_ШаблоныСообщений.ПечатныеФормыИВложения КАК C2D_ШаблоныСообщенийПечатныеФормыИВложения
		|ГДЕ
		|	C2D_ШаблоныСообщенийПечатныеФормыИВложения.Ссылка = &Шаблон
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	C2D_ШаблоныСообщенийПараметры.ПредставлениеПараметра,
		|	C2D_ШаблоныСообщенийПараметры.ТипПараметра,
		|	C2D_ШаблоныСообщенийПараметры.ИмяПараметра
		|ИЗ
		|	Справочник.C2D_ШаблоныСообщений.Параметры КАК C2D_ШаблоныСообщенийПараметры
		|ГДЕ
		|	C2D_ШаблоныСообщенийПараметры.Ссылка = &Шаблон";
		...
		Если СведенияОШаблоне.Количество() > 0 Тогда
			...
			Если СтрокаСведенийОШаблоне.ПредназначенДляSMS  Тогда
				Результат.ТипШаблона = "SMS"
			ИначеЕсли СтрокаСведенийОШаблоне.ПредназначенДляЭлектронныхПисем Тогда
				Результат.ТипШаблона = "Письмо"
			Иначе
				Результат.ТипШаблона = "Chat2Desk"
			КонецЕсли;
...
// Параметры:
//  Шаблон - СправочникОбъект.C2D_ШаблоныСообщений
...
//   * ФорматПисьма - ПеречислениеСсылка.C2d_СпособыРедактированияЭлектронныхПисем
//   * ПолноеИмяТипаНазначения - Строка
//   * Назначение - Строка
//   * ТипШаблона - Строка
//
Процедура УстановитьПараметрыШаблона(Шаблон, Результат)
	...
	ИначеЕсли Шаблон.ТипТекстаПисьма = Перечисления.C2d_СпособыРедактированияЭлектронныхПисем.HTML Тогда
...
Функция ПараметрыИзТекстаСообщения(ПараметрыШаблона) Экспорт
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" или ПараметрыШаблона.ТипШаблона = "Chat2Desk" Тогда
...
Функция ЗаполнитьПараметрыСообщения(ПараметрыШаблона, ПараметрыТекстаСообщения, ПараметрыОтправки)
	...
	Если ПараметрыОтправки.ДополнительныеПараметры.Свойство("ПроизвольныеПараметры") Тогда
		Для каждого ПроизвольныйПараметрШаблона Из ПараметрыТекстаСообщения Цикл
			...
			Если КлючПараметра = C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров() Тогда
				ПроизвольныеРеквизиты = ПараметрыТекстаСообщения[C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()];
			...
			Если КлючПараметра = C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов() Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ЗначениеПроизвольногоПараметра <> Неопределено Тогда
				...
				Если ТипЗнч(ЗначениеПроизвольногоПараметра) = Тип("Строка")
					...
					ПроизвольныеРеквизиты = ПараметрыТекстаСообщения[C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()];
					Если ТипЗнч(ПроизвольныеРеквизиты ) = Тип("Соответствие") Тогда
						ПараметрыТекстаСообщения[C2D_ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()][КлючПараметра] = ЗначениеПроизвольногоПараметра;
					КонецЕсли;
				Иначе
	...
	Если ПараметрыТекстаСообщения[C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов()] <> Неопределено Тогда
		ЗаполнитьОбщиеРеквизиты(ПараметрыТекстаСообщения[C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов()]);
		Сообщение.ЗначенияОбщихРеквизитов = ПараметрыТекстаСообщения[C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов()];
	КонецЕсли;
...
Процедура ДобавитьРеквизитПоМетаданнымОбъекта(Реквизиты, Реквизит, Префикс, СведенияОСпискеРеквизитов, СведенияОИсключаемыхРеквизитах)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		Попытка
			...
			КонецЕсли;
		Исключение
			ИмяРеквизита  = "~КИ."+Реквизит.Наименование;
			Представление = Строка(Реквизит);
			Тип           = Новый ОписаниеТипов("Строка");
			Формат        = "";
		КонецПопытки;
...
// Параметры:
//  ПараметрыТекстаСообщения - см. C2D_ШаблоныСообщенийСлужебный.ПараметрыИзТекстаСообщения
//  ОшибочныеРеквизиты - Массив
//  СведенияОШаблоне - см. C2D_ШаблоныСообщенийСлужебный.СведенияОШаблоне
//  Префикс - Строка
//
...
Функция ОбщиеРеквизиты(Реквизиты) Экспорт
	...
	УзелРеквизитов = Реквизиты.Строки.Найти(C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов(), "Имя");
	Если УзелРеквизитов = Неопределено Тогда
		УзелРеквизитов = Реквизиты.Строки.Добавить();
		УзелРеквизитов.Имя = C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов();
...
Функция ОпределитьОбщиеРеквизиты() Экспорт
	...
	РазвернутьРеквизит(C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов() + ".ТекущийПользователь", ОбщиеРеквизитыСтроки.Строки,, СписокИсключаемыхРеквизитов);
...
Процедура ДобавитьОбщийРеквизит(ОбщиеРеквизиты, Имя, Представление, Тип = Неопределено)
	...
	НовыйРеквизит.Имя = C2D_ШаблоныСообщений.ИмяУзлаОбщихРеквизитов() + "." + Имя;
...
Процедура ЗаполнитьПолучателейСообщения(ПараметрыОтправки, ПараметрыШаблона, Результат, МенеджерОбъекта)
	...
	КонецЕсли;
	Если ПараметрыШаблона.ТипШаблона = "Chat2Desk" Тогда
		...
		Результат.Получатель = Новый СписокЗначений;
			Для каждого Получатели Из Получатели Цикл
				Если ЗначениеЗаполнено(Получатели.Id) Тогда
					Результат.Получатель.Добавить(Получатели.Id, Получатели.Представление);
				КонецЕсли;
			КонецЦикла;
	ИначеЕсли ПараметрыШаблона.ТипШаблона = "Письмо" Тогда
		Получатели = СформироватьПолучателейПоУмолчанию(ПараметрыОтправки.Предмет, ПараметрыШаблона.ТипШаблона);
		C2D_ШаблоныСообщенийПереопределяемый.ПриЗаполненииПочтыПолучателейВСообщении(Получатели, ПараметрыШаблона.ПолноеИмяТипаНазначения, ПредметСообщения);
		Если МенеджерОбъекта <> Неопределено Тогда
	...
	Иначе
		...
		Получатели = СформироватьПолучателейПоУмолчанию(ПараметрыОтправки.Предмет, ПараметрыШаблона.ТипШаблона);
		C2D_ШаблоныСообщенийПереопределяемый.ПриЗаполненииТелефоновПолучателейВСообщении(Получатели, ПараметрыШаблона.ПолноеИмяТипаНазначения, ПредметСообщения);
		Если МенеджерОбъекта <> Неопределено Тогда
...
Функция СформироватьПолучателейПоУмолчанию(Предмет, ТипШаблона);
	...
	ИначеЕсли СтрСравнить(ТипШаблона, "Письмо") = 0 Тогда 
		Получатели.Колонки.Добавить("Адрес", Новый ОписаниеТипов("Строка"));
		ИмяКолонки = "Адрес";
	Иначе
		Получатели.Колонки.Добавить("Id", Новый ОписаниеТипов("Число"));
		ИмяКолонки = "Id";
	КонецЕсли;
	...
	Если Предмет = Неопределено Тогда
		Возврат Получатели;
	КонецЕсли;
	Если СтрСравнить(ТипШаблона, "Chat2Desk") = 0 Тогда
		РегистрыСведений.C2D_Клиенты.ВернутьПолучателейДляПредмета(Предмет, Получатели);
	Иначе	
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
			МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
...
Процедура СформироватьСообщениеВФоне(ПараметрыВызоваСервера, АдресХранилища) Экспорт
	...
	ПараметрыОтправки = ПараметрыВызоваСервера.ПараметрыОтправки; // см. C2D_ШаблоныСообщенийКлиентСервер.КонструкторПараметровОтправки
	...
	Иначе
		...
		ПараметрыШаблона = C2D_ШаблоныСообщений.СформироватьПараметрыОтправки(ПараметрыОтправки.Шаблон, ПараметрыОтправки.Предмет,
