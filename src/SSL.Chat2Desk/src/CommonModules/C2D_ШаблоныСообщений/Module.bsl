...
//
// Параметры:
//  Шаблон                   - СправочникСсылка.C2D_ШаблоныСообщений - ссылка на шаблон сообщения.
...
Функция СформироватьСообщение(Шаблон, Предмет, УникальныйИдентификатор, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Возврат C2D_ШаблоныСообщенийСлужебный.СформироватьСообщение(ПараметрыОтправки);
...
Функция СформироватьСообщениеИОтправить(Шаблон, Предмет, УникальныйИдентификатор, ДополнительныеПараметры = Неопределено) Экспорт
	ПараметрыОтправки = СформироватьПараметрыОтправки(Шаблон, Предмет, УникальныйИдентификатор, ДополнительныеПараметры);
	Возврат C2D_ШаблоныСообщенийСлужебный.СформироватьСообщениеИОтправить(ПараметрыОтправки);
КонецФункции
...
Процедура СформироватьСписокРеквизитовПоСКД(Реквизиты, Макет) Экспорт
	C2D_ШаблоныСообщенийСлужебный.РеквизитыПоСКД(Реквизиты, Макет);
КонецПроцедуры
...
Процедура ЗаполнитьРеквизитыПоСКД(Реквизиты, Предмет, ПараметрыШаблона) Экспорт
	C2D_ШаблоныСообщенийСлужебный.ЗаполнитьРеквизитыПоСКД(Реквизиты, Предмет, ПараметрыШаблона);
КонецПроцедуры
...
//  ПараметрыШаблона - см. C2D_ШаблоныСообщений.ОписаниеПараметровШаблона.
//
// Возвращаемое значение:
//  СправочникСсылка.C2D_ШаблоныСообщений - ссылка на созданный шаблон.
//
Функция СоздатьШаблон(Наименование, ПараметрыШаблона) Экспорт
	
	ПараметрыШаблона.Вставить("Наименование", Наименование);
	
	Шаблон = Справочники.C2D_ШаблоныСообщений.СоздатьЭлемент();
...
//   * ФорматПисьма    - ПеречислениеСсылка.C2d_СпособыРедактированияЭлектронныхПисем- формат письма HTML или обычный текст.
...
Процедура РазвернутьРеквизит(Имя, Узел, СписокРеквизитов = "", ИсключаяРеквизиты = "") Экспорт
	C2D_ШаблоныСообщенийСлужебный.РазвернутьРеквизит(Имя, Узел, СписокРеквизитов, ИсключаяРеквизиты);
КонецПроцедуры
...
//         - СправочникСсылка.C2D_ШаблоныСообщений
...
Функция ПараметрыШаблона(Знач Шаблон) Экспорт
	...
	Если ТипЗнч(Шаблон) <> Тип("ДанныеФормыСтруктура") 
		И ТипЗнч(Шаблон) <> Тип("СправочникСсылка.C2D_ШаблоныСообщений") Тогда
	...
	Результат = C2D_ШаблоныСообщенийСлужебный.ПараметрыШаблона(Шаблон);
...
Функция ВставитьПараметрыВСтрокуСогласноТаблицеПараметров(Знач ШаблонСтроки, ВставляемыеЗначения, Знач Префикс = "") Экспорт
	Возврат C2D_ШаблоныСообщенийСлужебный.ВставитьПараметрыВСтрокуСогласноТаблицеПараметров(ШаблонСтроки, ВставляемыеЗначения, Префикс);
КонецФункции
...
Функция ПараметрыИзТекстаСообщения(ПараметрыШаблона) Экспорт
	Возврат C2D_ШаблоныСообщенийСлужебный.ПараметрыИзТекстаСообщения(ПараметрыШаблона);
КонецФункции
...
Процедура ЗаполнитьОбщиеРеквизиты(ОбщиеРеквизиты) Экспорт
	C2D_ШаблоныСообщенийСлужебный.ЗаполнитьОбщиеРеквизиты(ОбщиеРеквизиты);
КонецПроцедуры
...
//
// Параметры:
//  ШаблонСсылка - СправочникСсылка.C2D_ШаблоныСообщений - ссылка на элемент справочника шаблоны сообщений.
// 
// Возвращаемое значение:
//  Булево - если Истина, то ссылка является элементом справочника шаблоны сообщений.
//
Функция ЭтоШаблон(ШаблонСсылка) Экспорт
	Возврат ТипЗнч(ШаблонСсылка) = Тип("СправочникСсылка.C2D_ШаблоныСообщений");
КонецФункции
...
Процедура ПриОпределенииОбъектовСКомандамиСозданияНаОсновании(Объекты) Экспорт
	...
	Объекты.Добавить(Метаданные.Справочники.C2D_ШаблоныСообщений);
...
Процедура ПриДобавленииКомандСозданияНаОсновании(Объект, КомандыСозданияНаОсновании, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Взаимодействия") Тогда
		Если Объект = Метаданные.Документы["ЭлектронноеПисьмоИсходящее"] Тогда
			Справочники.C2D_ШаблоныСообщений.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
		КонецЕсли;
	КонецЕсли;
...
// 
// Параметры:
//  Шаблон - СправочникСсылка.C2D_ШаблоныСообщений
...
Функция СформироватьПараметрыОтправки(Шаблон, Предмет, УникальныйИдентификатор, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПараметрыОтправки = C2D_ШаблоныСообщенийКлиентСервер.КонструкторПараметровОтправки(Шаблон.Ссылка, Предмет, УникальныйИдентификатор);
...
//   * Ссылка - СправочникСсылка.C2D_ШаблоныСообщенийПрисоединенныеФайлы
